...
function generateReadmeContent(task: Task): string {
  ...
  const difficultyLabel =
    task.difficulty === 'beginner' ? '🟢 초급' :
    task.difficulty === 'intermediate' ? '🟡 중급' :
    '🔴 고급'
...
> **코드 분량 제한**: AI 코드 리뷰의 정확도를 위해 **핵심 파일 3~5개, 총 300줄 이내**로 작성해주세요.
>
> 너무 많은 파일이나 코드를 제출하면 리뷰 품질이 저하될 수 있습니다.
> 보일러플레이트보다 **핵심 로직 구현**에 집중해주세요!
